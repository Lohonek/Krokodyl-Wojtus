            ...
            s
                ...
                .addIntegerOption((o) =>
                    o
                        .setName('duration')
                        .setDescription(
                            'Duration for the warning to last (in minutes).'
                        )
                )
        )
        .addSubcommand((s) =>
    ...
    run: async (client, interaction) => {
        try {
            ...
            if (subcommand === 'add') {
                ...
                const reason =
                    options.getString('reason') || 'No reason specified.'
                const duration = options.getInteger('duration') || 0

                if (user.bot || user.id === interaction.user.id)
                    return interaction.reply({
                        content:
                            '`❌` You cannot add a warning to a bot or yourself.',
                        ephemeral: true,
                    })
                ...
                try {
                    ...
                    if (index >= 0 && index < warnings.length) {
                        ...
                        interaction.reply({
                            content: `\`✅\` Warning ${index + 1} deleted for ${
                                user.tag
                            }.`,
                            ephemeral: true,
                        })
                        ...
                        if (deletedWarning.Duration > 0) {
                            const endTimestamp = new Date(
                                deletedWarning.Timestamp.getTime() +
                                    deletedWarning.Duration
                            )
